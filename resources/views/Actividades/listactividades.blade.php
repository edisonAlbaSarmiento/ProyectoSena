		...
		<section class="col-md-8 col-md-offset-1">
			<section class="card-panel hoverable">
				<div class="panel panel-danger">
			  		<div class="panel-heading"><center><h3>Actividades</h3></center></div>
			   			<div class="panel-body">
			   			@if ($roles->nombrerol == 'Aprendiz')<!--Se Hace un condicion para que se muestre si es el rol que me muestre lo correspondiente al rol y si no que siga con la condicion --> 
							{!! Form::open(['route' => 'actividadE/search', 'method' => 'post', 'novalidate', 'class' => 'form-inline alto']) <!--Me busca la ruta para que me pueda hacer el metodo de busca-->!!}
							...
							<!--Articulo Donde estan los botones-->
								...
								<a href="{{ route('actividades.index') }}" class="btn-floating btn-large waves-effect waves-light tooltipped"data-position="bottom" data-delay="50" data-tooltip="Mostrar Todo"><i class="fa fa-list-alt" aria-hidden="true"></i></a>
								<a href="{{ route('actividades.create') }}" class="btn-floating btn-large waves-effect waves-light tooltipped"data-position="bottom" data-delay="50" data-tooltip="Crear"><i class="fa fa-plus" aria-hidden="true"></i></a>
							</article>
						@endif
						{!! Form::close() !!}		
						<article class="form-group">
							...
							<table class="bordered highlight responsive-table tabla">
								...
								<tbody>
								@foreach($actividad as $Actividades)<!--se crea una condicion para que me muestre las llaves foraneas y el paginamiento-->
										<tr>
											<td>{{$Actividades->id}}</td>
										    <td>{{$Actividades->actividad }}</td>
							    			<td>{{$Actividades->evidencias }}</td>
							    			<td>{{$Actividades->fecha }}</td>
							    			<td>{{$Actividades->lugar }}</td>
							    		    <td>{{$Actividades->nombreaprendiz}}</td>
											<td>
												<a href="{{route ('actividades.edit',['id'=> $Actividades->id])}}" class="btn-floating
